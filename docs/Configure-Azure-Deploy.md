...
Remember to replace those values with your own in `src/Neudesic.IdentityServer.Admin/appsettings.json` before first publish -
...
Last step before deploy - we need to update `src/Neudesic.IdentityServer.STS.Identity/appsettings.json` and modify following lines:
